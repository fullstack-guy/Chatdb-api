...
const getCreateTableStatement = async (pool, fullTableName) => {
  ...
  `;
    ...
    console.log(`Executing query: ${query} with parameters: schema=${tableSchema}, table=${tableName}`);  // Debugging line
    ...
    const columnDefinitions = rows.map(row => {
        let columnDefinition = `${row.column_name} ${row.data_type.toUpperCase()}`;
...
const handler = async (request, reply) => {
    try {
        ...
        const createTableStatement = await getCreateTableStatement(pool, table_name);
        ...
        if (validateQuery(sql).valid) {
            try {
                ...
                const appendedPrompt = createPrompt([createTableStatement], question, err.message, sql);
                ...
                if (validateQuery(fixedSQL).valid) {
                    try {
                        ...
                        reply.status(500).send({ "error": `An unexpected error occurred: ${secondErr.message}` });
                    }
                } else {
